...
public class PokerHandsTest {
    ...
    @Test
    void should_return_white_win_when_compare_four_of_a_kind_given_black_3H3D3S3C5H_and_white_7H7D7S7C2D() {
        //given
        String input = "3H 3D 3S 3C 5H 7H 7D 7S 7C 2D";

        //when
        String result = pokerHands.playGame(input);

        //then
        assertThat(result).isEqualTo("White wins. - with four of a kind: 7");
    }

    @Test
    void should_return_black_win_when_compare_straight_flush_given_black_3H4H5H6H7H_and_white_7C7D7S7C2D() {
        //given
        String input = "3H 4H 5H 6H 7H 2D 8S 8C 8D 8H";

        //when
        String result = pokerHands.playGame(input);

        //then
        assertThat(result).isEqualTo("Black wins. - with straight flush: 7");
    }

    @Test
    void should_return_white_win_when_compare_straight_flush_given_black_3H4H5H6H7H_and_white_9HTHJHQHKH() {
        //given
        String input = "3H 4H 5H 6H 7H 9H TH JH QH KH";

        //when
        String result = pokerHands.playGame(input);

        //then
        assertThat(result).isEqualTo("White wins. - with straight flush: King");
    }
}